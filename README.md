...
Складываем последние цифры первых двух чисел
...
Определяем последние две цифры числа: x % 100
Проверяем условия:
...
отсутствие break позволяет выполнять последующие case
...
System.out.print("*") выводит звёздочки без перевода строки
System.out.println() переходит на новую строку после каждой строки звёздочек
...
Идем до первого элемента
Уменьшаем индекс на каждой итерации
...
Создаем новый массив result, длина которого равна сумме длин массивов arr1 и arr2
